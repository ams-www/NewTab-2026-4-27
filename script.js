...
function renderIcons() {
  ...
  filtered.forEach((item) => {
    ...
    if (wallpaperActive) {
      card.className = 'bg-white/85 dark:bg-slate-800/85 backdrop-blur-sm p-4 rounded-2xl border border-slate-100/50 dark:border-slate-700/50 flex flex-col items-center gap-3 relative cursor-pointer select-none transition-transform duration-200 overflow-hidden';
    } else {
      card.className = 'bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-100 dark:border-slate-700 flex flex-col items-center gap-3 relative cursor-pointer select-none transition-transform duration-200 overflow-hidden';
    }
